...
{
	void * storage = MemoryManager::newMalloc(s, Globals::MANAGER);
	return storage;
}
...
MemoryManager & MemoryManager::getInstance()
{
	if(MemoryManager::count==0) {
		...
		stackMarker = (unsigned int) MemoryManager::newMalloc(dataToAllocate, Globals::ALLOCATOR);
		...
		baseSingleFrameMarker = (unsigned int) MemoryManager::newMalloc(dataToAllocate, Globals::ALLOCATOR);
		...
		// clear the log file
		if(Globals::verbosityLog>=1) Log::clearLog(Globals::MALLOC_LOGFILE);
...
{
	if(Globals::verbosityLevel >= 4) cout << "NEW MALLOC ALLOCATES: " << s << " bytes" << endl;
	if(Globals::verbosityLog>=2) {
		Log::addToLog(Globals::MALLOC_LOGFILE, "MALLOC,");
		...
		Log::addToLog(Globals::MALLOC_LOGFILE, sizeAllocated);
		Log::addToLog(Globals::MALLOC_LOGFILE, ",");
		Log::addToLog(Globals::MALLOC_LOGFILE, flag);
		Log::addToLog(Globals::MALLOC_LOGFILE, "\n");
	}
	return malloc(s);
}
...
{
	if(Globals::verbosityLevel >= 4) cout << "NEW FREE FREES: " << ptr << endl;
	free(ptr);
}
...
void * MemoryManager::allocateOnStack(unsigned int s) 
{
	...
	if(Globals::verbosityLevel >= 4) cout << "STACK ALLOCATOR ALLOCATES: " << s << " bytes" << endl;
...
void * MemoryManager::allocateOnSingleFrameAllocator(unsigned int s) 
{
	...
	if(Globals::verbosityLevel >= 4) cout << "SINGLE FRAME ALLOCATOR ALLOCATES: " << s << " bytes" << endl;
...
{
	singleFrameMarker = baseSingleFrameMarker;
	if(Globals::verbosityLevel>=4) cout << "SINGLE FRAME ALLOCATOR CLEARED" << endl;
}
...
void MemoryManager::logMaxMemoryUsage(void) 
{
	...
	if(Globals::verbosityLog>=1) {
		...
		Log::addToLog(Globals::MEMORYMANAGER_LOGFILE, "MAX MEMORY USAGE: ");
